...
public class HandlerDataSourceAop {
    //@within在类上设置
    //@annotation在方法上进行设置
    @Pointcut("@within(com.spring.boot.mybatisplus.druid.dynamic.datasource.configuration.DynamicSwitchDataSource) || @annotation(com.spring.boot.mybatisplus.druid.dynamic.datasource.configuration.DynamicSwitchDataSource)")
    ...
    public void doBefore(JoinPoint joinPoint) {
        ...
        log.info("AOP动态切换数据源，className" + joinPoint.getTarget().getClass().getName() + "methodName" + method.getName() + ";dataSourceKey:" + (StringUtils.isEmpty(dataSourceKey) ? "默认数据源" : dataSourceKey));
    }

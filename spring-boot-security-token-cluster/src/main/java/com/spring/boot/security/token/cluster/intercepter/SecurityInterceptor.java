package com.spring.boot.security.token.cluster.intercepter;
...
//        Arrays.stream(cookies).forEach(cookie -> logger.debug("cookie.getName: {} getValue: {} getComment: {} getDomain: {} getMaxAge: {} getVersion: {}", cookie.getName(), cookie.getValue(), cookie.getComment(), cookie.getDomain(), cookie.getMaxAge(), cookie.getVersion()));
        ...
        //从Redis中获取token
        String tokenOfRedis = stringRedisTemplate.opsForValue().get("token-" + username);
        logger.debug("从Redis中获取token: {}", token);

        //Redis中Token为空表示用户未登陆
        if (StringUtils.isEmpty(tokenOfRedis)) {
            logger.debug("用户未登陆");
            return false;
        }

        //Redis中Token与用户请求中Token不匹配时表示token过期，需要用户重新登录
        if (!tokenOfRedis.equals(token)) {
            logger.debug("用户Token已过期");
            return false;
        }
